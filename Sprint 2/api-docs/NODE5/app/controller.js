...
router.get('/dhtTemperature', (request, response, next) => {

    let sum = ArduinoDataDhtTemperature.List.reduce((a, b) => a + b, 0);
    let average = (sum / ArduinoDataDhtTemperature.List.length).toFixed(2);
    let sumHour = ArduinoDataDhtTemperature.ListHour.reduce((a, b) => a + b, 0);
    let averageHour = (sumHour / ArduinoDataDhtTemperature.ListHour.length).toFixed(2);

    response.json({
        data: ArduinoDataDhtTemperature.List,
        total: ArduinoDataDhtTemperature.List.length,
        average: isNaN(average) ? 0 : average,
        dataHour: ArduinoDataDhtTemperature.ListHour,
        totalHour: ArduinoDataDhtTemperature.ListHour.length,
        averageHour: isNaN(averageHour) ? 0 : averageHour
    });
